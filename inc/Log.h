...
public:
	...
	void flush();
	LogWriter *setLogWriter(LogWriter f);
...
#define INFO(fmt, ...)                                                           \
	{                                                                            \
	 if (logger._level<=Log::L_INFO) 	logger.tfl(__SHORT_FILE__, __LINE__).printf(fmt, ##__VA_ARGS__).flush(); \
	}
#define WARN(fmt, ...)                                                           \
	{                                                                            \
		if (logger._level<=Log::L_WARN) logger.tfl(__SHORT_FILE__, __LINE__).printf(fmt, ##__VA_ARGS__).flush(); \
	}
#define DEBUG(fmt, ...)                                                          \
	{                                                                            \
		if (logger._level<=Log::L_DEBUG) logger.tfl(__SHORT_FILE__, __LINE__).printf(fmt, ##__VA_ARGS__).flush(); \
	}
#define ERROR(fmt, ...)                                                          \
	{                                                                            \
		if (logger._level<=Log::L_ERROR)  logger.tfl(__SHORT_FILE__, __LINE__).printf(fmt, ##__VA_ARGS__).flush(); \
	}
